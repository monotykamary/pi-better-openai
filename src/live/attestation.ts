import type { DeviceCheckTokenResult, LiveNativeBindings } from "./native.ts";
...
function buildClientAttestation(result: DeviceCheckTokenResult): string {
...
export async function generateCodexAttestation(
  native: Pick<LiveNativeBindings, "deviceCheckGenerateToken">,
): Promise<string | undefined> {
  if (process.platform !== "darwin" || process.arch !== "arm64") return undefined;
  let result: DeviceCheckTokenResult;
  try {
    result = await native.deviceCheckGenerateToken();
  } catch {
    return undefined;
  }
